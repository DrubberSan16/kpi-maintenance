...
export class KpiMaintenanceService implements OnModuleInit, OnModuleDestroy {
  ...
  private async ensureAutomaticWorkOrderAlertWithManager(
    ...
    const alertData = {
      equipo_id: workOrder.equipment_id ?? null,
      tipo_alerta: 'ORDEN_TRABAJO_GENERADA',
    ...
    if (existing) {
      ...
      if (resolvedState === 'CERRADA') {
        await this.applyClosedWorkOrderAlertOutcome(existing, workOrder, snapshots);
      }
      return {
    ...
    if (resolvedState === 'CERRADA') {
      await this.applyClosedWorkOrderAlertOutcome(created, workOrder, snapshots);
      return {
